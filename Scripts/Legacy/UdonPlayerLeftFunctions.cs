...
{
    public class UdonPlayerLeftFunctions : UdonSharpBehaviour
    {
        /// <summary>
        /// 需要调用的UdonBehaviour
        /// </summary>
        ...
        public void OnPlayerLeft_()
        {
            ...
            || OnPlayerLeft_VRCPlayerApi == null
            || isLocalOnly && !OnPlayerLeft_VRCPlayerApi.isLocal)
                return;
            for (int i = 0; i < udonBehaviours.Length; i++)
            {
                ...
                if (i < joinVRCPlayerApiValueName.Length)
                {
                    ...
                    {
                        if (isOnlyPlayerName)
                            udonBehaviours[i].SetProgramVariable(joinVRCPlayerApiValueName[i], OnPlayerLeft_VRCPlayerApi.displayName);
                        else
                            udonBehaviours[i].SetProgramVariable(joinVRCPlayerApiValueName[i], OnPlayerLeft_VRCPlayerApi);
                    }
                }
